...
function addShrapnel(level, gameCtx, match) {
    const center = getMatchCenter(match);

    const bricks = [];
    for (let i = 0; i < 4; i++) {
        const brick = gameCtx.entityFactory.brickShrapnel();
...
function handleY({entity, match, resolver, gameCtx, level}) {
    if (entity.vel.y > 0) {
        ...
        if (entity.traits.has(Player)) {
            if(entity.powerup === "big" || entity.powerup === "flower") {
                const grid = resolver.matrix;
                grid.delete(match.indexX, match.indexY);
                addShrapnel(level, gameCtx, match);
            }
        }
